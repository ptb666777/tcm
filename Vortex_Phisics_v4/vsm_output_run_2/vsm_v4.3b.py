...
def step_pins(pins, psi, gx, gy, gz):
    for p in pins:
        ...
        # Force scaled by pin's own omega (charge sign)
        # Positive pin falls toward negative wells, away from positive hills
        # Negative pin falls toward positive hills, away from negative wells
        # True symmetric charge interaction - Grok/Claude fix
        force = p['omega'] * np.array([-float(gx[ix,iy,iz]),
                                        -float(gy[ix,iy,iz]),
                                        -float(gz[ix,iy,iz])])
        p['vel'] += force*DT*tr
        p['vel'] *= DAMP
...
SCENARIOS = {
    ...
    '10v10':    {'pins':(sphere_pins(10,8,[half-20,half,half], 1.0,'P')+
                 ...
                 'steps':100000,'desc':'10 pos vs 10 neg 3D spheres'},
