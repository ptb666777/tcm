...
# Auto core count: 0 = all minus 1 (keeps one core free for Windows)
# Hard cap at physical core count to prevent thrashing
physical_cores = mp.cpu_count()
if CPU_CORES == 0:
    CPU_CORES = max(1, physical_cores - 1)
else:
    CPU_CORES = min(CPU_CORES, physical_cores - 1)
...
if __name__ == '__main__':
    # Print config summary here (not at module level - workers reimport module on Windows)
    print(f"VDC v12 3D | N={N} | steps={steps} | cores={CPU_CORES}/{mp.cpu_count()}")
    ...
    # Pin main process away from core 0 on Windows if psutil available
    try:
        import psutil
        proc = psutil.Process()
        all_cores = list(range(mp.cpu_count()))
        worker_cores = all_cores[1:]  # leave core 0 for Windows
        proc.cpu_affinity(worker_cores)
        print(f"CPU affinity set: using cores {worker_cores[0]}-{worker_cores[-1]}, core 0 reserved")
    except Exception:
        print(f"CPU affinity not set (install psutil for core pinning: pip install psutil)")
    print()
    if SEED == 0:
        np.random.seed(None)
    else:
