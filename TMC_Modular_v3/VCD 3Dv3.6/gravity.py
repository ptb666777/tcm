...
class GravityModule(VDCModule):
    ...
    def step(self, state, cfg):
        ...
        # Post-recombination: pressure is gone, gravity is unopposed
        # Boost grav_str so clouds can actually fall toward each other
        # Pre-recombination: radiation pressure fights gravity, keep base strength
        from matter_state import epoch_at_least, RECOMBINATION
        grav_now = self.grav_str * 4.0 if epoch_at_least(state.epoch, RECOMBINATION) else self.grav_str

        # Apply tidal force scaled by local complexity
        # Matter that is more complex both pulls and is pulled more strongly
        local_scale = grav_now * complexity
