...
class WaveModule(VDCModule):
    ...
    def step(self, state, cfg):
        ...
        # Pin-modulated wave speed - slower near pins (thicker substrate)
        # This creates standing wave nodes at pin locations
        pin_density = state.fields.get('pin_density',
                                        np.zeros(state.grid.shape))
        pin_max = pin_density.max()
        if pin_max > 1e-10:
            pin_norm = pin_density / pin_max
            speed_mod = 1.0 - 0.4 * pin_norm
            state.wave_v += (c2 * speed_mod - c2) * self._laplacian(state.wave)
        else:
            pin_norm = np.zeros_like(state.wave)

        # Wave-matter coupling scaled by phase AND pin proximity
        phase = state.fields.get('matter_phase', np.ones(state.grid.shape))
        phase_frac = phase / max(phase.max(), 1.0)
        pin_couple = 1.0 + pin_norm * 2.0
        couple_str = self.wave_couple * phase_frac * pin_couple
        ...
        pin_count = len(state.pin_field.pins) if hasattr(state, 'pin_field') else 0
        return {
            'wave_active': 1,
            'wave_E':      float(wave_E),
